...
export default function Home() {
  ...
  useEffect(() => {
    ...
    ctx.add("(min-width: 768px)", () => {
      ...
      // Recalculate ScrollTrigger on next tick
      setTimeout(() => {
    ...
    <main ref={containerRef} className="relative w-full bg-brand-black min-h-screen overflow-x-hidden">
      ...
      <section className="relative w-full h-[100svh] min-h-[600px] overflow-hidden flex flex-col items-center justify-center">
        <motion.div style={{ y: yHeroVideo }} className="absolute inset-0 w-full h-full z-0 will-change-transform">
          <div className="absolute inset-0 bg-brand-black/45 z-10"></div>
          <video
            ref={heroVideoRef}
            autoPlay
            muted
          ...
          >
            <source src="/1789804326602393.mp4" type="video/mp4" />
          </video>
        </motion.div>
        ...
        <motion.div style={{ y: yHeroTitle }} className="relative z-20 flex flex-col items-center text-center px-4 sm:px-6 max-w-4xl mx-auto mt-auto mb-24 sm:mb-32">

          {/* Upgraded Mobile Action Buttons */}
          <div className="flex flex-col sm:flex-row items-center gap-3 sm:gap-5 w-full sm:w-auto max-w-xs sm:max-w-none">
            <Link
              href="/showreel"
              className="w-full sm:w-auto px-7 py-3.5 rounded-full bg-white text-black text-xs font-mono tracking-widest uppercase font-bold hover:bg-brand-light active:scale-95 transition-all shadow-[0_0_35px_rgba(255,255,255,0.35)] flex items-center justify-center gap-2.5 group cursor-pointer"
              data-cursor="play"
            >
            ...
            <Link
              href="/work"
              className="w-full sm:w-auto px-7 py-3.5 rounded-full bg-black/70 sm:bg-white/5 backdrop-blur-xl border border-white/30 hover:border-white text-white text-xs font-mono tracking-widest uppercase font-medium hover:bg-white/15 active:scale-95 transition-all flex items-center justify-center gap-2 group cursor-pointer"
            >
        ...
        {/* Scroll Indicator */}
        <div className="absolute bottom-6 sm:bottom-10 left-1/2 -translate-x-1/2 z-20 flex flex-col items-center gap-2 pointer-events-none text-white/60">
          <span className="text-[9px] uppercase tracking-[0.3em] font-mono">Scroll</span>
          <div className="w-[1px] h-8 sm:h-12 bg-white/20 overflow-hidden">
            <motion.div
              animate={{ y: [0, 48] }}
          ...
          <div className="flex flex-col md:flex-row justify-between items-start gap-8 sm:gap-16">
            ...
            <div className="md:w-2/3 relative w-full aspect-[16/10] sm:aspect-[4/5] md:aspect-[16/9] overflow-hidden group rounded-2xl border border-white/10 shadow-2xl">
              <Image
                ...
                fill
                className="object-cover transition-transform duration-1000 group-hover:scale-105 filter contrast-[1.05]"
              />
      ...
      >
        <div className="w-full flex flex-col justify-center h-full max-h-screen py-4 sm:py-6 md:py-8 space-y-4 md:space-y-6">
          ...
          <div className="w-full px-4 sm:px-6 lg:px-14 flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4 z-20 shrink-0">
            ...
            {/* Right Side: Fast Link & Mobile Controls */}
          ...
          {/* Horizontal Carousel in the middle */}
          <div 
            ref={wrapperRef} 
            data-lenis-prevent
              ...
              >
                <div className="relative w-full h-full overflow-hidden rounded-2xl border border-white/15 bg-brand-dark shadow-[0_20px_50px_rgba(0,0,0,0.9)] group-hover:border-white/45 transition-all duration-500">
                  <Image
                    ...
                    fill
                    priority={idx < 2}
                    sizes="(max-width: 768px) 90vw, 55vh"
                    className="object-cover filter contrast-[1.05] brightness-95 group-hover:brightness-105 group-hover:scale-105 transition-transform duration-700 ease-out"
                  />
